...
def relpath(filefolder,filename):
    return os.path.join(os.path.dirname(__file__), filefolder,filename)

def show_example(im1, im2, mask, blended_img):
...
def blending_example1():
    im1 = read_image(relpath("FinalExample1","JawsPoster.jpg"), 2)
    im2 = read_image(relpath("FinalExample1","ChiuauaJaws.jpg"), 2)
    mask = read_image(relpath("FinalExample1","Mask1.jpg"),1).astype(np.bool)
    blended_img = rgb_blending(im1, im2, mask, 5, 5, 9)
    show_example(im1, im2, mask, blended_img)
    return im1, im2, mask, blended_img

def blending_example2():
    # Just to note - I could have cut around the edges of the queen image,
    # but then since the poster has 4 faces you would still be able to use them
    # and this is probably the most iconic Queen band album covers.
    # In fact I blurred the mask eve more just to cover the band faces well.
    im1 = read_image(relpath("FinalExample2","QueenBand.jpg"), 2)
    im2 = read_image(relpath("FinalExample2","ActualQueen.jpg"), 2)
    mask = read_image(relpath("FinalExample2","Mask2.jpg"),1).astype(np.bool)
    blended_img = rgb_blending(im1, im2, mask, 5, 3, 15)
    show_example(im1, im2, mask, blended_img)
    return im1, im2, mask, blended_img